...
if [ ${#CHANGED_FILES_JS[@]} -eq 0 ]; then
    ...
    printf "%s\n" "${CHANGED_FILES_JS[@]}"
    printf "%s\0" "${CHANGED_FILES_JS[@]}" | xargs -0 -I {} sh -c 'if [ -f "{}" ]; then ./node_modules/.bin/eslint "{}" ; fi'
fi
...
if [ ${#CHANGED_FILES_CSS[@]} -eq 0 ]; then
    ...
    printf "%s\n" "${CHANGED_FILES_CSS[@]}"
    printf "%s\0" "${CHANGED_FILES_CSS[@]}" | xargs -0 -I {} sh -c 'if [ -f "{}" ]; then ./node_modules/.bin/stylelint "{}" ; fi'
fi
...
if [ ${#CHANGED_FILES_PHP[@]} -eq 0 ]; then
   echo "No changed PHP files found."
    ...
    printf "%s\n" "${CHANGED_FILES_PHP[@]}"
    printf "%s\0" "${CHANGED_FILES_PHP[@]}" | xargs  -0 -I {} sh -c 'if [ -f "{}" ]; then php -l -n "{}" ; fi' | (! grep -v "No syntax errors detected" )
fi
